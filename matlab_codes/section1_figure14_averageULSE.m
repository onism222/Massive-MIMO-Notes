...
for k = 1:numberOfRealizations
    ...
    SE_NLoS_montecarlo(:,k) = log2(1 + SNR*cumsum(abs(fadingRealizationsDesired(:,k)).^2) ./ ...
        ( betabar*SNR*(abs(cumsum(conj(fadingRealizationsDesired(:,k)).*fadingRealizationsInterfering(:,k))).^2)./...
        cumsum(abs(fadingRealizationsDesired(:,k)).^2) +1));
...
xlabel(['Number of antennas M (from ', num2str(Mmin), ' to ', num2str(Mmax), ')']);
